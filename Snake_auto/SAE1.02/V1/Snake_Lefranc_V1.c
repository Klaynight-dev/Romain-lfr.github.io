...
char calculerDirectionAuto(int xTete, int yTete, int xPomme, int yPomme);
...
int main()
{
    ...
    while (1)
    {
        ...
        direction = calculerDirectionAuto(tabX[0], tabY[0], lesPommesX[nbPomme], lesPommesY[nbPomme]);  
...
void progresser(int tabX[], int tabY[], char plateau[HAUTEUR][LARGEUR], bool*collision, bool*pommeManger, char *direction)
{
    ...
    int xPomme = lesPommesX[nbPomme];
    int yPomme = lesPommesY[nbPomme];
    ...
    if (*pommeManger) {
        *direction = calculerDirectionAuto(tabX[0], tabY[0], xPomme, yPomme);
    }
    for (i = TAILLE; i > 0; i--)
    {
        tabX[i] = tabX[i - 1];
...
/**
 * \fn char calculerDirectionAuto(int xTete, int yTete, int xPomme, int yPomme)
 * \brief Calcule la prochaine direction automatique pour aller vers la pomme
 * \param xTete : Coordonnée X de la tête du serpent
 * \param yTete : Coordonnée Y de la tête du serpent
 * \param xPomme : Coordonnée X de la pomme
 * \param yPomme : Coordonnée Y de la pomme
...
char calculerDirectionAuto(int xTete, int yTete, int xPomme, int yPomme) {
    if (xTete < xPomme) {
        return DROITE;  // Se déplacer vers la droite
    } 
    else if (xTete > xPomme) {
        return GAUCHE;  // Se déplacer vers la gauche
    } 
    else if (yTete > yPomme) {
        return HAUT;  // Se déplacer vers le haut
    } 
    else if (yTete < yPomme) {
        return BAS;  // Se déplacer vers le bas
    }
